...
Процедура ПутьСохраненияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Фильтр = НСтр("ru = 'Обновление'; en = 'Update'")
    + "(*.cf)|*.cf|" 
	+ НСтр("ru = 'Обновление'; en = 'Update'") +"(*.cfu)|*.cfu";
...
Процедура СделатьКопию(Команда)   
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("СделатьКопиюЗавершение", ЭтотОбъект), "Работа программы будет завершена. Все не сохраненные данные могут быть утеряны. Обновить программу?",РежимДиалогаВопрос.ДаНет); 
...
Процедура СделатьКопиюЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда 
		...
		Объект.КаталогОбновления + "\update.bat", // имя
		...
		Текст.ЗаписатьСтроку("%path_1C% CONFIG /F%name_base_backup% /N%user_1C% /P%pswd_1C% /UpdateCfg %cfu_file% /UpdateDBCfg /Out update.log");  
		Текст.ЗаписатьСтроку("TIMEOUT /T 60 /NOBREAK ");
		Текст.ЗаписатьСтроку("%path_1C% enterprise /F%name_base_backup% /N%user_1C% /P%pswd_1C%");
		Текст.Закрыть();  
		НачатьЗапускПриложения(Новый ОписаниеОповещения("СделатьКопиюЗавершениеЗавершение", ЭтотОбъект),"update.bat", Объект.КаталогОбновления);  
	Иначе
	КонецЕсли;
