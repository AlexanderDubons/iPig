...
Процедура СделатьКопию(Команда)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("СделатьКопиюЗавершение", ЭтотОбъект),
		"Работа программы будет завершена. Все не сохраненные данные могут быть утеряны. Делать копию базы данных?",
...
#Если Не ВебКлиент Тогда
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		Текст = Новый ЗаписьТекста(Объект.ПутьСохранения + "\backup.bat", // имя
		...
		НазваниеФайла = Строка(_НастройкиКонфигурацииНаСервере.ПолучитьУНПНаСервере()) + "_" + Формат(ТекущаяДата(),
			"ДФ=yyyy-MM-dd") + ".dt";
		...
		Текст.Закрыть();
		НачатьЗапускПриложения(Новый ОписаниеОповещения("СделатьКопиюЗавершениеЗавершение", ЭтотОбъект), "backup.bat",
			Объект.ПутьСохранения);
...
Процедура ЗагрузитьКопию(Команда)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("ЗагрузитьКопиюЗавершение", ЭтотОбъект),
		"Работа программы будет завершена. Все не сохраненные данные могут быть утеряны. Загрузить копию базы данных?",
		РежимДиалогаВопрос.ДаНет);
...
#Если Не ВебКлиент Тогда
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		Текст = Новый ЗаписьТекста(Объект.ПутьЗагрузки + "backup.bat", // имя
...
Процедура ПутьЗагрузкиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ДиалогОткрытия.Показать(Новый ОписаниеОповещения("ОткрытьЗавершение", ЭтаФорма, Новый Структура("Диалог",
		ДиалогОткрытия)));
КонецПроцедуры
