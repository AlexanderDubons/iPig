...
Процедура ЗаполнитьОтборПоНеделямНаСервере()  
	...
	Пока РезультатЗапроса.Следующий() Цикл
		...
		НоваяСтрока.НомерНедели 	= _НастройкиКонфигурацииНаСервере.РасчетНомераНедели(РезультатЗапроса.ДатаСобытия,СдвигНедели);    
		...
		//--------------------------------------------------------------		
		НоваяСтрока.НачалоНедели 	= _НастройкиКонфигурацииНаСервере.ДатаПоНомеруНедели(Формат(ДатаКорректировки,"ДФ=гггг") + "-W" + Строка(НоваяСтрока.НомерНедели), СдвигНедели); 
	...
	Для Каждого Стр ИЗ РезультатЗапроса Цикл 
		...
		Для Каждого СтрКолонка ИЗ РезультатЗапросаКолонка Цикл   
			Если СтрКолонка.ТипСобытия = Перечисления.ТипСобытия.Случка Тогда 
				НоваяСтрока[СтрЗаменить(УбратьЛишниеСимволы(СтрКолонка.Название)," ","_")+Строка(СтрКолонка.Код)] =  Стр.ДатаСлучки +  СтрКолонка.Дни * 24 * 60 *60;
			ИначеЕсли СтрКолонка.ТипСобытия = Перечисления.ТипСобытия.Опорос Тогда 
				НоваяСтрока[СтрЗаменить(УбратьЛишниеСимволы(СтрКолонка.Название)," ","_")+Строка(СтрКолонка.Код)] =  Стр.ДатаОпороса - СтрКолонка.Дни * 24 * 60 *60
			КонецЕсли;
		КонецЦикла;						   
	КонецЦикла; 	
	...
	Для Каждого СтрТР ИЗ ТаблицаРезультат Цикл
		Элементы.ГруппаОсеменения.СписокВыбора.Добавить(СтрТР.НомерНедели, Строка(СтрТР.НомерНедели) + " ("+ Формат(СтрТР.НачалоНедели,"ДФ=dd.MM.yy") + "-" + Формат(СтрТР.КонецНедели,"ДФ=dd.MM.yy")+")");
	КонецЦикла;
...
Процедура ПечатьКарточкуНаСервере(ТабДок)
	...
	ОбластьЗаголовок.Параметры.ГруппаОсеменения = Элементы.ГруппаОсеменения.СписокВыбора.НайтиПоЗначению(ГруппаОсеменения);
...
Процедура ПечатьКарточкиСвиноматкиНаСервере(ТабДок)
	...
	Для Каждого Запись ИЗ ТЗГруппы Цикл 
		РегистрыСведений.КарточкаСвиноматки.ПолучитьДанныеОпоросНаСервере(Запись.НомерЖивотного,Запись.ДатаВвода,ТЧОпоросы);
		РегистрыСведений.КарточкаСвиноматки.ПолучитьДанныеСлучкаНаСервере(Запись.НомерЖивотного,Запись.ДатаВвода,ТЧСлучка);	
		РегистрыСведений.КарточкаСвиноматки.ПолучитьДанныеСупоросностьНаСервере(Запись.НомерЖивотного,Запись.ДатаВвода,ТЧСупоросность,Запись.НомерОпороса);
		РегистрыСведений.КарточкаСвиноматки.ПолучитьДанныеАбортаНаСервере(Запись.НомерЖивотного,Запись.ДатаВвода,ТЧАборт,Запись.НомерОпороса);
		РегистрыСведений.КарточкаСвиноматки.ПолучитьДанныеПодсосныеПоросятаНаСервере(Запись.НомерЖивотного,Запись.ДатаВвода,ТЧПодсосныеПоросята);
		РегистрыСведений.КарточкаСвиноматки.ПолучитьДанныеОтъемаНаСервере(Запись.НомерЖивотного,Запись.ДатаВвода, ТЧОтъем); 
		РегистрыСведений.КарточкаСвиноматки.ПолучитьКоличествоОпоросовНаСервере(Запись.НомерЖивотного,Запись.ДатаВвода,Запись.НомерОпороса);	
...
Процедура ЗаполнитьОбщуюТаблицуНаСервере(Запись)
	...
	РегистрыСведений.КарточкаСвиноматки.ЗаполнитьОбщуюТаблицу(Запись, ТЧОбщая, ТЧСлучка, ТЧСупоросность, ТЧАборт, ТЧОпоросы, ТЧОтъем, ТЧБрак, Ложь);
...
Процедура ОбщиеНастройки(Команда) 
	...
	ОткрытьФорму("РегистрСведений.НастройкаКарточкиГруппы.Форма.ФормаНастроек",,ЭтотОбъект,,,,ОписаниеОповещение,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры   
...
Процедура ЗаполнитьТаблицуКолонками()
	...
	Пока РезультатЗапроса.Следующий() Цикл  
		...
		УдаляемыеРеквизиты.Добавить("СписокГрупп." + СтрЗаменить(УбратьЛишниеСимволы(РезультатЗапроса.Название)," ","_")+Строка(РезультатЗапроса.Код));  
		Попытка
	...
	РезультатЗапроса = Запрос.Выполнить().Выбрать();
	Пока РезультатЗапроса.Следующий() Цикл
		СоздатьКолонкуНаСервере(СтрЗаменить(УбратьЛишниеСимволы(РезультатЗапроса.Название)," ","_")+Строка(РезультатЗапроса.Код),РезультатЗапроса.Название,ОписаниеТипаДата,"СписокГрупп",Элементы.СписокГрупп);
	КонецЦикла;
 КонецПроцедуры
