...
Процедура ПолучитьДанныеНаСервере()
	Попытка
		...
		Соединение.ConnectionString = "driver={SQL Server};" + "server=" + "192.168.5.135,1433" + ";" + "uid=" + "def"
			+ ";" + "pwd=" + "def" + ";" + "database=" + "AgroSoft_PV" + ";";
		...
		Если Выборка.BOF = Ложь Тогда
			...
			Пока Выборка.EOF = Ложь Цикл
				...
				Если Справочники.ЖурналЛечения.НайтиПоКоду(КодМедикамента) = Справочники.ЖурналЛечения.ПустаяСсылка() Тогда
					...
					НоваяЗапись.Код 			   	= КодМедикамента;
					НоваяЗапись.Диагноз 	   	  	= Справочники.Диагноз.НайтиПоКоду(Выборка.Fields(
						"DCode").Value);
					НоваяЗапись.Медикаменты			= Справочники.Медикаменты.НайтиПоКоду(Выборка.Fields(
						"MCode").Value);
					НоваяЗапись.Наименование		= НоваяЗапись.Диагноз.Наименование + " / "
						+ НоваяЗапись.Медикаменты.Наименование;
		...
		Если Выборка.BOF = Ложь Тогда
			...
			Пока Выборка.EOF = Ложь Цикл
				...
				Если Справочники.Работник.НайтиПоКоду(КодРаботника) = Справочники.Работник.ПустаяСсылка() Тогда
					...
					НоваяЗапись.Наименование 	   = Выборка.Fields("Name").Value + " " + Выборка.Fields(
						"Initials").Value;
					НоваяЗапись.Записать();
				КонецЕсли;
				Выборка.MoveNext();
		...
		Если Выборка.BOF = Ложь Тогда
			...
			Пока Выборка.EOF = Ложь Цикл
				Если Выборка.Fields("AnimalType").Value = 1 Тогда  //Родитель   
					...
					Если Справочники.КарточкаРодители.НайтиПоКоду(КодРодителя)
						= Справочники.КарточкаРодители.ПустаяСсылка() И ЗначениеЗаполнено(Выборка.Fields(
						"AnimalNumber").Value) Тогда
						...
						НоваяЗапись.КодПороды 			= Справочники.Порода.НайтиПоКоду(Выборка.Fields(
							"BreedId").Value);
						Если ЗначениеЗаполнено(Выборка.Fields("DGTagId").Value) Тогда
							Если Справочники.IDНомер.НайтиПоНаименованию(Выборка.Fields("DGTagId").Value)
								= Справочники.IDНомер.ПустаяСсылка() Тогда
							...
							КонецЕсли;
							НоваяЗапись.DGTagId = Справочники.IDНомер.НайтиПоНаименованию(Выборка.Fields(
								"DGTagId").Value);
						КонецЕсли;
						НоваяЗапись.Записать();
					КонецЕсли;
			...
			Пока Выборка.EOF = Ложь Цикл
				Если Выборка.Fields("AnimalType").Value = 0 Тогда   //Животное фермы
					...
					Если ЗначениеЗаполнено(Выборка.Fields("TagId").Value) Тогда
						Если Справочники.IDНомер.НайтиПоНаименованию(Выборка.Fields("TagId").Value)
							= Справочники.IDНомер.ПустаяСсылка() Тогда
							НоваяЗаписьСпр 				= Справочники.IDНомер.СоздатьЭлемент();
					...
					Если Выборка.Fields("FatherId").Value <> -1 Тогда
						...
						Если Справочники.КарточкаРодители.НайтиПоКоду(ДанныеОтец.НомерЖивотногоРодителя)
							= Справочники.КарточкаРодители.ПустаяСсылка() И ЗначениеЗаполнено(
							ДанныеОтец.НомерЖивотногоРодителя) Тогда
						...
						КонецЕсли;
						НоваяЗапись.РодителиОтец = Справочники.КарточкаРодители.НайтиПоКоду(
							ДанныеОтец.НомерЖивотногоРодителя);
					КонецЕсли;
					Если Выборка.Fields("MotherId").Value <> -1 Тогда
						ДанныеМать = ПолучитьНомерЖивотногоПоИД(Выборка.Fields("MotherId").Value);
						Если Справочники.КарточкаРодители.НайтиПоКоду(ДанныеМать.НомерЖивотногоРодителя)
							= Справочники.КарточкаРодители.ПустаяСсылка() И ЗначениеЗаполнено(
							ДанныеМать.НомерЖивотногоРодителя) Тогда
							...
							НоваяЗаписьРод.Записать();
						КонецЕсли;
						НоваяЗапись.РодителиМать = Справочники.КарточкаРодители.НайтиПоКоду(
							ДанныеМать.НомерЖивотногоРодителя);
					КонецЕсли;
				...
				ИначеЕсли Выборка.Fields("AnimalType").Value = 1 Тогда  //Родитель   
					КодРодителя = Выборка.Fields("AnimalNumber").Value;
					Если Справочники.КарточкаРодители.НайтиПоКоду(КодРодителя)
						<> Справочники.КарточкаРодители.ПустаяСсылка() Тогда
						НоваяЗапись 					= Справочники.КарточкаРодители.НайтиПоКоду(
							КодРодителя).ПолучитьОбъект();
						НоваяЗапись.Код 				= КодРодителя;
						ПолучитьКодЖивотного(КодРодителя);
						НоваяЗапись.НомерЖивотного 		= Справочники.НомерЖивотного.НайтиПоКоду(КодРодителя);
						НоваяЗапись.Пол 				= ПолучитьПолЖивотного(Выборка);
						...
						Если ДанныеОтец <> Неопределено Тогда
							НоваяЗапись.НомерЖивотногоМать 	= Справочники.КарточкаРодители.НайтиПоКоду(
								ДанныеОтец.НомерЖивотногоРодителя);
						КонецЕсли;
						ДанныеМать = ПолучитьНомерЖивотногоПоИД(Выборка.Fields("MotherId").Value);
						Если ДанныеМать <> Неопределено Тогда
							НоваяЗапись.НомерЖивотногоОтец 	= Справочники.КарточкаРодители.НайтиПоКоду(
								ДанныеМать.НомерЖивотногоРодителя);
						КонецЕсли;
						Если ЗначениеЗаполнено(Выборка.Fields("DGTagId").Value) Тогда
							Если Справочники.IDНомер.НайтиПоНаименованию(Выборка.Fields("DGTagId").Value)
...
Процедура ПолучитьКодЖивотного(Знач КодРодителя)
	...
	Если ЗначениеЗаполнено(КодРодителя) И Справочники.НомерЖивотного.НайтиПоКоду(КодРодителя)
		= Справочники.НомерЖивотного.ПустаяСсылка() Тогда
		...
		Если Выборка.BOF = Ложь Тогда
			...
			Пока Выборка.EOF = Ложь Цикл
				...
				Если НаборЗаписейОсеменение.Количество() = 1 Тогда
					Для Каждого Стр Из НаборЗаписейОсеменение Цикл
						Если Стр.НомерЖивотногоХряк = Справочники.НомерЖивотного.ПустаяСсылка() Тогда
							...
							Стр.КодПородыХряк      = _НастройкиКонфигурацииНаСервере.ПолучитьПородуХрякаНаСервере(
								Стр.НомерЖивотногоХряк);
						ИначеЕсли Стр.НомерЖивотногоХряк1 = Справочники.НомерЖивотного.ПустаяСсылка() Тогда
							Стр.НомерЖивотногоХряк1	=  СпрКодХряка;
							Стр.КодПородыХряк1      = _НастройкиКонфигурацииНаСервере.ПолучитьПородуХрякаНаСервере(
								Стр.НомерЖивотногоХряк1);
						ИначеЕсли Стр.НомерЖивотногоХряк2 = Справочники.НомерЖивотного.ПустаяСсылка() Тогда
							Стр.НомерЖивотногоХряк2 	=  СпрКодХряка;
							Стр.КодПородыХряк2      = _НастройкиКонфигурацииНаСервере.ПолучитьПородуХрякаНаСервере(
								Стр.НомерЖивотногоХряк2);
						КонецЕсли;
					КонецЦикла;
				...
				Иначе
					...
					НоваяЗапись.КодПородыХряк      = _НастройкиКонфигурацииНаСервере.ПолучитьПородуХрякаНаСервере(
						НоваяЗапись.НомерЖивотногоХряк);
					//---------------------------------			
		...
		Если Выборка.BOF = Ложь Тогда
			...
			Пока Выборка.EOF = Ложь Цикл
				...
				Если ЗначениеЗаполнено(Выборка.Fields("LitterId").Value) Тогда
					Если Справочники.IDГнезда.НайтиПоНаименованию(Выборка.Fields("LitterId").Value)
						= Справочники.IDГнезда.ПустаяСсылка() Тогда
	...
	Попытка
		...
		Если Выборка.BOF = Ложь Тогда
			...
			Пока Выборка.EOF = Ложь Цикл
				...
				НоваяЗапись.РодителиМатьПорода          = Справочники.Порода.НайтиПоКоду(Выборка.Fields(
					"MatherBreedId").Value); 
				//----------------------------------
	...
	Попытка
		...
		Если Выборка.BOF = Ложь Тогда
			...
			Пока Выборка.EOF = Ложь Цикл
				...
				НоваяЗапись.РодителиОтец 		= Справочники.НомерЖивотного.НайтиПоКоду(
					ДанныеОтец.НомерЖивотногоРодителя);
