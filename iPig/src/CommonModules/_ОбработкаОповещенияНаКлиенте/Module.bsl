...
Процедура СделатьКопиюЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.ОК Тогда 
		...
		ДополнительныеПараметры.КаталогВременныхФайлов + "\update.bat", // имя
		...
		Текст.Закрыть();  
		НачатьЗапускПриложения(Новый ОписаниеОповещения("СделатьКопиюЗавершениеОбновления", _ОбработкаОповещенияНаКлиенте),"update.bat", ДополнительныеПараметры.КаталогВременныхФайлов);  
	Иначе
	КонецЕсли;
...
Процедура ОткрытьФормуСвиноматки(НомерЖивотного, ДатаВвода) Экспорт
	Попытка
		ОткрытьФорму("РегистрСведений.КарточкаСвиноматки.Форма.ФормаЗаписи", Новый Структура("Ключ, ФормаОткрыта", _НастройкиКонфигурацииНаСервере.ПолучитьКлючЗаписи(НомерЖивотного, ДатаВвода),Истина),,,,,,);
	Исключение 
