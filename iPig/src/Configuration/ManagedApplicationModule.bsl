...
Асинх Процедура ПриНачалеРаботыСистемы()    
   //_НастройкиКонфигурацииНаСервере.РасчетНомераНедели(Дата(2024,1,8),1);
	...
	УстановитьЗаголовокКлиентскогоПриложения("iPig " + _НастройкиКонфигурацииНаСервере.ПолучитьВерсиюКонфигурацииНаСервере() + " / " 
	+ _НастройкиКонфигурацииНаСервере.ПолучитьНаименованиеОрганизацииНаСервере() 
	+ " / Лицензия до " + Формат(_НастройкиКонфигурацииНаСервере.ПолучитьДатуЛицензии(),"ДФ=dd.MM.yyyy"));  
...
Асинх  Процедура ПроверкаДанныхОрганизации()
	...
	Если НЕ РезультатПроверки  ИЛИ Отказ Тогда
		...
		ОткрытьФорму("ОбщаяФорма.ДанныеОрганизации",,,,,,Оповещение,РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);  
	Иначе 
		...
		ИначеЕсли _НастройкиКонфигурацииНаСервере.СделатьКопиюБазыНаСервере() Тогда  
			...
			НачатьЗапускПриложения(Новый ОписаниеОповещения("СделатьКопиюЗавершениеЗавершение", _ОбработкаОповещенияНаКлиенте,ПутьСохранения+"\"+НазваниеФайла),"backup.bat", ПутьСохранения);
		КонецЕсли;
	...
	Для Каждого Файл Из МассивФайлов Цикл
		...
		Если Файл.ЭтоФайл() Тогда     
			...
			Если   ВерсияОбновления <> _НастройкиКонфигурацииНаСервере.ПолучитьВерсиюКонфигурацииНаСервере()  Тогда 				
				...
				КонецЦикла; 
				ДопПараментры = Новый Структура("КаталогВременныхФайлов, ПутьКФайлуОбновления",КаталогВременныхФайлов,ПутьКФайлуОбновления);
				ОтвентНавопрос = Неопределено;
            	ПоказатьВопрос(Новый ОписаниеОповещения("СделатьКопиюЗавершение", _ОбработкаОповещенияНаКлиенте,ДопПараментры), "Будет произведено обновление программы",РежимДиалогаВопрос.ОК); 
