...
Процедура ЗаписатьОсеменения(Команда)
	Для Каждого Запись ИЗ ТЧДанные Цикл  
		Попытка
			СоздатьЗаписьЖурналаОсемененияНаСервере(Запись.ДатаСобытия, Запись.НомерЖивотного, Запись.КодПороды, Запись.ДатаВвода,
			Запись.Хряк, Запись.ОценкаСлучки, Запись.Статус, Запись.КодПородыХряк, Запись.Работник, Запись.Хряк1, Запись.КодПородыХряк1, Запись.Хряк2, Запись.КодПородыХряк2 );
...
&НаСервере
Процедура СоздатьЗаписьЖурналаОсемененияНаСервере(Знач ДатаСлучка, Знач НомерЖивотного, Знач КодПороды, Знач ДатаВвода, Знач НомерЖивотногоХряк, Знач ОценкаСлучки, Знач Статус, Знач КодПородыХряк, Знач Работник
