...
Процедура ПередЗаписью(Отказ, Замещение)
	Для Каждого Запись ИЗ ЭтотОбъект Цикл
		Если Запись.Живорожденные = 0 И
			Запись.Мертвые = 0 И
			Запись.Муммии = 0 И
			Запись.Свинки = 0 И
			Запись.Слаборожденные  = 0 Тогда 
		...
		Иначе
			...
			Если НаборЗаписей.Количество() = 1 Тогда   
				Если НаборЗаписей[0].ПоследнееСобытие = Перечисления.ПоследнееСобытие.Случка И  НаборЗаписей[0].ДатаПоследнегоСобытия >= Запись.Период И Запись.НомерОпороса = 0 Тогда
					Сообщить("Для свиноматки " + Запись.НомерЖивотного + " Дата опороса должна быть больше " + Формат(НаборЗаписей[0].ДатаПоследнегоСобытия,"ДФ=dd.MM.yy"));
					Если НЕ Константы.ОтключитьКонтрольОшибок.Получить() Тогда
						Отказ = Истина;	
						ВызватьИсключение("Для свиноматки " + Запись.НомерЖивотного + " Дата опороса должна быть больше " + Формат(НаборЗаписей[0].ДатаПоследнегоСобытия,"ДФ=dd.MM.yy"));   
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;	
