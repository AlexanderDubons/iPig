...
Процедура ЗаписатьОпоросы(Команда)
	Для Каждого Запись ИЗ ТЧОпорос Цикл  
		Попытка
			...
			Запись.МассаВсего, Запись.СреднийВес, Запись.ПереведеноПоросят, Запись.IDГнезда,Запись.Место,Запись.ТолщинаШпика,Запись.КоличествоСосков);
...
&НаСервере
Процедура СоздатьЗаписиОпоросНаСервере(ДатаОпороса,НомерЖивотного, КодПороды, ДатаВвода,Живорожденные,Мертвые,Слаборожденные,Муммии,Свинки,МассаВсего,СреднийВес,ПереведеноПоросят,IDГнезда,
											Место, ТолщинаШпика, КоличествоСосков)	
	Перем Записи;
