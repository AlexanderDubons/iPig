...
Функция СписокВТЗнаСервере()
	...
	Если Статистика Тогда
		...
		Если КоличествоОпоросов > 0  Тогда
			Живорожденные 		= Строка(ЖиворожденныеКоличество) + " / " + Строка(ОКР(ЖиворожденныеКоличество/КоличествоОпоросов,2));
			Мертвые			 	= Строка(МертвыеКоличество) + " / " + Строка(ОКР(МертвыеКоличество/КоличествоОпоросов,2));
			Слаборожденные		= Строка(СлаборожденныеКоличество) + " / " + Строка(ОКР(СлаборожденныеКоличество/КоличествоОпоросов,2));
			Муммии				= Строка(МуммииКоличество) + " / " + Строка(ОКР(МуммииКоличество/КоличествоОпоросов,2));
			Свинки				= Строка(СвинкиКоличество) + " / " + Строка(ОКР(СвинкиКоличество/КоличествоОпоросов,2));
			МассаВсего			= Строка(МассаВсегоКоличество) + " / " + Строка(ОКР(МассаВсегоКоличество/КоличествоОпоросов,2));
			СреднийВес			= Строка(СреднийВесКоличество) + " / " + Строка(ОКР(СреднийВесКоличество/КоличествоОпоросов,2));
		КонецЕсли;
...
Процедура СписокПослеУдаления(Элемент)
	...
	_НастройкиКонфигурацииНаСервере.ЗаписьДанныхРегистраВЖурналРегистрацийПриУдалении(Элемент.ТекущаяСтрока,УдалениеНомерЖивотного,УдалениеДатаСобытия,"Удаление");
КонецПроцедуры
