...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если (Запись.Период <> ТекущаяДатаВвода ИЛИ Запись.НомерЖивотного <> ТекущийНомерЖивотного) И (ТекущийНомерЖивотного <> Справочники.НомерЖивотного.ПустаяСсылка())Тогда  			
...
Процедура НомерЖивотногоОткрытие(Элемент, СтандартнаяОбработка)
	...
	ОткрытьФорму("РегистрСведений.КарточкаСвиноматки.Форма.ФормаЗаписи",Новый Структура("Ключ", ПолучитьКлючЗаписи(Запись.НомерЖивотного, Запись.Период)),Элемент,,,,,РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс); 
КонецПроцедуры
...
&НаСервереБезКонтекста
Процедура СменитьНомерЖивотногоРегистрНаСервере(ДатаВвода, Знач НаборЗаписей, НомерЖивотного, Знач НовыйНомерЖивотного, Знач НоваяДатаВвода )
...
Процедура РодителиМатьОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)  
	...
	Если ДанныеПоСвиноматкам.Количество() <> 0 Тогда
		...
		КонецЕсли; 
		ВыбранноеЗначение = ПолучитьДанныеРодителяНаСервере(НомерЖивотного, ДатаВвода, ПредопределенноеЗначение("Перечисление.Пол.Свинка"));
	Иначе
...
Процедура РодителиОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ДанныеПоСвиноматкам.Количество() <> 0 Тогда
		...
		Если ВыбСтрокаМеню <> Неопределено Тогда 
			...
			ДатаВвода	  	= Реквизиты.ДатаВвода;
		КонецЕсли; 
		ВыбранноеЗначение = ПолучитьДанныеРодителяНаСервере(НомерЖивотного, ДатаВвода, ПредопределенноеЗначение("Перечисление.Пол.Хряк"));
	Иначе
