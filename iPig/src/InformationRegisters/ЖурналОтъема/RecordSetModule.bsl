	...
	Для Каждого Запись Из ЭтотОбъект Цикл
		...
		Если НаборЗаписей.Количество() = 1 Тогда
			Если НаборЗаписей[0].ПоследнееСобытие = Перечисления.ПоследнееСобытие.Случка И Запись.НомерОпороса = 0 Тогда
				...
				КонецЕсли;
			ИначеЕсли НаборЗаписей[0].ПоследнееСобытие = Перечисления.ПоследнееСобытие.Опорос И Запись.НомерОпороса = 0
				И НаборЗаписей[0].ДатаПоследнегоСобытия >= Запись.Период Тогда
				Сообщить("Для свиноматки " + Запись.НомерЖивотного + " Дата отъема должна быть больше " + Формат(
					НаборЗаписей[0].ДатаПоследнегоСобытия, "ДФ=dd.MM.yy"));
				Если Не Константы.ОтключитьКонтрольОшибок.Получить() Тогда
					Отказ = Истина;
					ВызватьИсключение ("Для свиноматки " + Запись.НомерЖивотного + " Дата отъема должна быть больше "
