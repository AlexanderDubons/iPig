...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)   
	...
	Если ПородаОтец = Справочники.Порода.НайтиПоРеквизиту("КодПороды","22") И ПородаМать = Справочники.Порода.НайтиПоРеквизиту("КодПороды","22") Тогда
		НоваяСтрока.Порода	= Справочники.Порода.НайтиПоРеквизиту("КодПороды","22");
	ИначеЕсли ПородаОтец = Справочники.Порода.НайтиПоРеквизиту("КодПороды","11") И ПородаМать = Справочники.Порода.НайтиПоРеквизиту("КодПороды","22") Тогда
	...
	Если Не ДанныеВБазе Тогда
		ЗаполнитьТЧДанныеНаКлиенте(НоваяСтрока.КоличествоПоросят, НоваяСтрока.СерийныйНомерСвинкиОт, НоваяСтрока.НомерЖивотного);
	КонецЕсли;
КонецПроцедуры
...
Процедура ТЧДанныеКоличествоПоросятПриИзменении(Элемент)
	...
	Если  НЕ ДанныеВБазе Тогда	
		ЗаполнитьТЧДанныеНаКлиенте(КоличествоПоросятDG,  ТекущаяСтрока.СерийныйНомерСвинкиОт, ТекущаяСтрока.НомерЖивотного);
	КонецЕсли;
...
&НаСервере
Процедура ЗаполнитьТЧДанныеНаКлиенте(Знач КоличествоПоросятDG, Знач СерийныйНомерСвинкиОт, Знач НомерЖивотного, Знач ПрефиксСвинки = "")
	...
	Для Счетчик = 1 По КоличествоПоросятDG Цикл
		...
		Иначе
			НоваяСтрока.IDНомер				= СтрЗаменить(КодПородыИФермы + Строка(СерийныйНомерСвинкиОт + Счетчик - 1)," ",""); 
		КонецЕсли;
		НоваяСтрока.Пол					= ПредопределенноеЗначение("Перечисление.Пол.Свинка");      
...
Процедура ТЧДанныеНомерТатуОтПриИзменении(Элемент) 
	...
	Если  НЕ ДанныеВБазе Тогда	
		ЗаполнитьТЧДанныеНаКлиенте(КоличествоПоросятDG,  ТекущаяСтрока.СерийныйНомерСвинкиОт, ТекущаяСтрока.НомерЖивотного, ПрефиксСвинки);
	КонецЕсли;
КонецПроцедуры   
