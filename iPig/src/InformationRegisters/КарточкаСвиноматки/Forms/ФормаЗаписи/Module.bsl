...
Процедура КарточкаНаСервере(ТабДок)
	...
	Для Каждого ВыборкаДетальныеЗаписи Из ТЧОбщая Цикл
		...
		Если ВыборкаДетальныеЗаписи.ДатаСлучка <> Дата(1, 1, 1) Тогда
			ОбластьСтрокаСобытия.Параметры.ПДатаОпороса = ВыборкаДетальныеЗаписи.ДатаСлучка
				+ _НастройкиКонфигурацииНаСервере.ПолучитьДниСупоросностиНаСервере() * (24 * 60 * 60);
		Иначе
...
Процедура ПеречитатьДанныеНаСервере()
	...
	РегистрыСведений.КарточкаСвиноматки.ПолучитьДанныеСупоросностьНаСервере(Запись.НомерЖивотного, Запись.Период,
		ТЧСупоросность, Запись.НомерОпороса);
	РегистрыСведений.КарточкаСвиноматки.ПолучитьДанныеАбортаНаСервере(Запись.НомерЖивотного, Запись.Период, ТЧАборт,
		Запись.НомерОпороса);
	РегистрыСведений.КарточкаСвиноматки.ПолучитьДанныеПодсосныеПоросятаНаСервере(Запись.НомерЖивотного, Запись.Период,
		ТЧПодсосныеПоросята);
	РегистрыСведений.КарточкаСвиноматки.ПолучитьДанныеОтъемаНаСервере(Запись.НомерЖивотного, Запись.Период, ТЧОтъем);
	РегистрыСведений.КарточкаСвиноматки.ПолучитьКоличествоОпоросовНаСервере(Запись.НомерЖивотного, Запись.Период,
		Запись.НомерОпороса);
...
Процедура ДобавитьОпорос(Команда)
	...
	ПараметрыФормы = Новый Структура("НомерЖивотного, НомерОпороса, ДатаВвода, Порода, ДатаРождения",
		Запись.НомерЖивотного, Запись.НомерОпороса + 1, Запись.Период, Запись.КодПороды, Запись.ДатаРождения);
...
Процедура ФормаЗакрыта(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	...
	РегистрыСведений.КарточкаСвиноматки.ПолучитьКоличествоОпоросовНаСервере(Запись.НомерЖивотного, Запись.Период,
		Запись.НомерОпороса);
	РегистрыСведений.КарточкаСвиноматки.ПолучитьДанныеВакцинацииНаСервере(Запись.НомерЖивотного, Запись.Период,
		ТЧВакцинация);
...
Процедура ПолучитьДанныеКлючевыеЦифрыОпоросаНаСервере()
	...
	Если НайденныеСтроки.Количество() > 1 Тогда
		ПустыеДни = (НайденныеСтроки[НайденныеСтроки.Количество() - 1].ДатаСлучка - НайденныеСтроки[0].ДатаСлучка)
			/ (60 * 60 * 24);
		Если ТЧКлючевыеЦифрыОпороса.Количество() > 0 Тогда
			ТЧКлючевыеЦифрыОпороса[0].ВсегоПотеряноКормодней = ТЧКлючевыеЦифрыОпороса[0].ВсегоПотеряноКормодней
				+ ПустыеДни;
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
//	|	И ЖурналОтъема.Мачеха = ЛОЖЬ";
	...
	Если Запись.ДатаВыбытие <> Дата(1, 1, 1) Тогда
		...
		Если ДатаПоследнегоОтъема <> Неопределено Тогда
			ПустыеКормодни = ПустыеКормодни + (НачалоДня(Запись.ДатаВыбытие) - НачалоДня(ДатаПоследнегоОтъема)) / (60
				* 60 * 24);
		КонецЕсли;
	КонецЕсли;
...
Процедура ДобавитьОтъем(Команда)
	Оповещение = Новый ОписаниеОповещения("ФормаЗакрыта", ЭтаФорма);
	ПараметрыФормы = Новый Структура("НомерЖивотного, НомерОпороса, ДатаВвода", Запись.НомерЖивотного,
		Запись.НомерОпороса + 1, Запись.Период);
...
Процедура ДобавитьОсеменение(Команда)
	Оповещение = Новый ОписаниеОповещения("ФормаЗакрыта", ЭтаФорма);
	ПараметрыФормы = Новый Структура("НомерЖивотного, НомерОпороса, ДатаВвода, Порода", Запись.НомерЖивотного,
		Запись.НомерОпороса + 1, Запись.Период, Запись.КодПороды);
...
Функция ПолучитьНомерНедели()
	...
	Если МассивДат.Количество() > 0 Тогда
		...
		Возврат _НастройкиКонфигурацииНаСервере.РасчетНомераНедели(МассивДат[0].ДатаСлучка,
			Константы.НачалоОтсчетаНомераНедели.Получить());
	Иначе
		Возврат 0;
	КонецЕсли;
...
КонецПроцедуры
&НаСервере
Процедура ЗаполнитьРегистрКлючевыЦифрыОпоросаНаСервере(Знач НомерОпороса, Знач НомерЖивотного, Знач Период,
	Знач ДатаОпороса, Знач ДниСупоросности, Знач ДатаОтъема, Знач ПодсосныеДни, Знач ПотеряноКормодней,
	Знач ДатаАборта, Знач ДатаСлучки, Знач ВозрастОтъема)
...
&НаСервере
Процедура ЗапистьДанныеОтъемНаСервере(ДатаОтъема, НомерОпороса, КоличествоОтъемышей, МассаВсего, СреднийВес, Мачеха,
	ВозрастПриПодсадке, ТолщинаШпика)
	НаборЗаписей = РегистрыСведений.ЖурналОтъема.СоздатьНаборЗаписей();
	НаборЗаписей.Отбор.НомерЖивотного.Установить(Запись.НомерЖивотного);
	НаборЗаписей.Отбор.ДатаВвода.Установить(Запись.Период);
	...
	Если НаборЗаписей.Количество() = 0 Тогда  
		...
		СоздатьЗаписиОтъемНаСервере(ДатаОтъема, 0, КоличествоОтъемышей, МассаВсего, СреднийВес, Мачеха,
			ВозрастПриПодсадке, ТолщинаШпика);
	Иначе
		Для Каждого Записи Из НаборЗаписей Цикл
		...
		КонецЦикла;
		НаборЗаписей.Записать();
		СоздатьЗаписиОтъемНаСервере(ДатаОтъема, НомерОпороса, КоличествоОтъемышей, МассаВсего, СреднийВес, Мачеха,
			ВозрастПриПодсадке, ТолщинаШпика);
	КонецЕсли;
...
&НаСервере
Процедура СоздатьЗаписиОтъемНаСервере(ДатаОтъема, НомерОпороса, КоличествоОтъемышей, МассаВсего, СреднийВес, Мачеха,
	ВозрастПриПодсадке, ТолщинаШпика)
	Перем Записи;
...
КонецПроцедуры
&НаСервере
Процедура ЗаписатьДанныеОпоросНаСервере(ДатаОпороса, НомерОпороса, Живорожденные, Мертвые, Слаборожденные, Муммии,
	Свинки, МассаВсего, СреднийВес, ПереведеноПоросят, IDГнезда, Место, КоличествоСосков, ТолщинаШпика)
	НаборЗаписей = РегистрыСведений.ЖурналОпороса.СоздатьНаборЗаписей();
	НаборЗаписей.Отбор.НомерЖивотного.Установить(Запись.НомерЖивотного);
	НаборЗаписей.Отбор.ДатаВвода.Установить(Запись.Период);
	...
	Если НаборЗаписей.Количество() = 0 Тогда  
		...
		СоздатьЗаписиОпоросНаСервере(ДатаОпороса, 0, Живорожденные, Мертвые, Слаборожденные, Муммии, Свинки, МассаВсего,
			СреднийВес, ПереведеноПоросят, IDГнезда, Место, КоличествоСосков, ТолщинаШпика);
	Иначе
		Для Каждого Записи Из НаборЗаписей Цикл
			НаборЗаписей.Удалить(Записи);
		КонецЦикла;
		НаборЗаписей.Записать();
		СоздатьЗаписиОпоросНаСервере(ДатаОпороса, НомерОпороса, Живорожденные, Мертвые, Слаборожденные, Муммии, Свинки,
			МассаВсего, СреднийВес, ПереведеноПоросят, IDГнезда, Место, КоличествоСосков, ТолщинаШпика);
	КонецЕсли;
КонецПроцедуры

&НаСервере
Процедура СоздатьЗаписиОпоросНаСервере(ДатаОпороса, НомерОпороса, Живорожденные, Мертвые, Слаборожденные, Муммии,
	Свинки, МассаВсего, СреднийВес, ПереведеноПоросят, IDГнезда, Место, КоличествоСосков, ТолщинаШпика)
...
&НаСервере
Процедура ЗаписатьДанныеСлучкиНаСервере(НомерОпороса, ДатаСлучка, НомерЖивотногоХряк, ОценкаСлучки, Статус, КодПороды,
	Работник, НомерЖивотногоХряк1, НомерЖивотногоХряк2, КодПороды1, КодПороды2)
	НаборЗаписей = РегистрыСведений.ЖурналОсеменения.СоздатьНаборЗаписей();
	НаборЗаписей.Отбор.НомерОпороса.Установить(ТекСтрокаФорма.НомерОпороса);
	...
	НаборЗаписей.Прочитать();
	Если НаборЗаписей.Количество() = 0 Тогда
		СоздатьЗаписьЖурналаОсемененияНаСервере(ДатаСлучка, НомерЖивотногоХряк, НомерОпороса, ОценкаСлучки, Статус,
			КодПороды, Работник, НомерЖивотногоХряк1, НомерЖивотногоХряк2, КодПороды1, КодПороды2);
	Иначе
		Для Каждого Записи Из НаборЗаписей Цикл
			НаборЗаписей.Удалить(Записи);
		КонецЦикла;
...
&НаСервере
Процедура СоздатьЗаписьЖурналаОсемененияНаСервере(Знач ДатаСлучка, Знач НомерЖивотногоХряк, Знач НомерОпороса,
	Знач ОценкаСлучки, Знач Статус, Знач КодПороды, Знач Работник, Знач НомерЖивотногоХряк1, Знач НомерЖивотногоХряк2,
	Знач КодПороды1, Знач КодПороды2)
...
Процедура ТЧОбщаяПередУдалением(Элемент, Отказ)
	ТекСтрокаФорма  = СтруктураДанных(Элемент);
	ТекДанные = Элемент.ТекущиеДанные;
	РезультатВопроса = Вопрос("Удалить данные " + СтрЗаменить(Элемент.ТекущийЭлемент.Имя, "ТЧОбщаяДата", "") + "?",
		...
		Если ТЧОбщая.Количество() = (ТчОбщая.Индекс(Элемент.ТекущиеДанные) + 1) Тогда
			Если СтрЗаменить(Элемент.ТекущийЭлемент.Имя, "ТЧОбщая", "") = "ДатаСлучка" Тогда
				Если ТекДанные.Живорожденные = 0 Тогда
					...
					_НастройкиКонфигурацииНаСервере.ЗаписьДанныхРегистраВЖурналРегистрацийПриУдалении(
						"РегистрСведенийНаборЗаписей.ЖурналОсеменения", Запись.НомерЖивотного, ТекДанные.ДатаСлучка,
						"УдалениеИзКарточки");
				Иначе
			...
			Если СтрЗаменить(Элемент.ТекущийЭлемент.Имя, "ТЧОбщая", "") = "ДатаОпороса" Тогда
				Если ТекДанные.ДатаОтъема = '00010101' Тогда
					...
					_НастройкиКонфигурацииНаСервере.ЗаписьДанныхРегистраВЖурналРегистрацийПриУдалении(
						"РегистрСведенийНаборЗаписей.ЖурналОпороса", Запись.НомерЖивотного, ТекДанные.ДатаОпороса,
						"УдалениеИзКарточки");
				Иначе
			...
			Если СтрЗаменить(Элемент.ТекущийЭлемент.Имя, "ТЧОбщая", "") = "ДатаОтъема" Тогда
				...
				_НастройкиКонфигурацииНаСервере.ЗаписьДанныхРегистраВЖурналРегистрацийПриУдалении(
					"РегистрСведенийНаборЗаписей.ЖурналОтъема", Запись.НомерЖивотного, ТекДанные.ДатаОтъема,
					"УдалениеИзКарточки");
			КонецЕсли;
			ПеречитатьДанныеНаСервере();
			ПеречитатьДанныеНаКлиенте();
...
Процедура ТЧОбщаяПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	Если Не МачехаФорма Тогда
		ТекДанные = Элемент.ТекущиеДанные;
		Если ТекДанные.ДатаСлучка <> Дата(1, 1, 1) Тогда
			ЗаписатьДанныеСлучкиНаСервере(ТекДанные.НомерОпороса, ТекДанные.ДатаСлучка, ТекДанные.НомерЖивотногоХряк,
				ТекДанные.ОценкаСлучки, ТекДанные.Статус, ТекДанные.НазваниеПородыХряк, ТекДанные.Работник,
				ТекДанные.НомерЖивотногоХряк1, ТекДанные.НомерЖивотногоХряк2, ТекДанные.НазваниеПородыХряк1,
				ТекДанные.НазваниеПородыХряк2);
			ПеречитатьДанныеНаСервере();
			ПеречитатьДанныеНаКлиенте();
		КонецЕсли;
		Если ТекДанные.ДатаУЗИ <> Дата(1, 1, 1) И ТекДанные.ДатаУЗИ > ТекДанные.ДатаСлучка Тогда
			ЗаписатьДанныеУЗИНаСервере(ТекДанные.НомерОпороса, ТекДанные.ДатаУЗИ, ТекДанные.РезультатУЗИ,
				ТекДанные.МестоСупоросность, ТекДанные.РаботникСупоросность, ТекДанные.ТолщинаШпикаСупоросность);
			ПеречитатьДанныеНаСервере();
			ПеречитатьДанныеНаКлиенте();
		...
		Если ТекДанные.ДатаОпороса <> Дата(1, 1, 1) И (ТекДанные.Живорожденные > 0 Или ТекДанные.Мертвые > 0) Тогда
			ЗаписатьДанныеОпоросНаСервере(ТекДанные.ДатаОпороса, ТекДанные.НомерОпороса, ТекДанные.Живорожденные,
				ТекДанные.Мертвые, ТекДанные.Слаборожденные, ТекДанные.Муммии, ТекДанные.Свинки,
				ТекДанные.МассаВсегоОпорос, ТекДанные.СреднийВесОпорос, ТекДанные.ПереведеноПоросят,
		...
		Если ТекДанные.ДатаОтъема <> Дата(1, 1, 1) Тогда
			ЗапистьДанныеОтъемНаСервере(ТекДанные.ДатаОтъема, ТекДанные.НомерОпороса, ТекДанные.КоличествоОтъемышей,
				ТекДанные.МассаВсего, ТекДанные.СреднийВес, ТекДанные.Мачеха, ТекДанные.ВозрастПриПодсадке,
				ТекДанные.ТолщинаШпикаОтъем);
			ПеречитатьДанныеНаСервере();
			ПеречитатьДанныеНаКлиенте();
		КонецЕсли; 
		//----------------------------------------------   
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	Для Каждого Стр Из ТЧОбщая Цикл
		...
		КонецЕсли;
		ЗаполнитьРегистрКлючевыЦифрыОпоросаНаСервере(Стр.НомерОпороса, Запись.НомерЖивотного, Запись.Период,
			ДатаОпороса, Стр.ПериодЛактации, Стр.ДатаОтъема, Стр.ПодсосныйПериод, Стр.ПустыеДни, Стр.ДатаАборта,
			Стр.ДатаСлучка, Стр.ВозрастПоросят);
	КонецЦикла;
	ОбновитьСтатусНаСервере();
КонецПроцедуры
...
Процедура ТЧОбщаяПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	Если КоличествоСтрок > 0 Тогда
		Если ТЧОбщая[КоличествоСтрок - 1].ДатаОтъема <> '00010101000000' Тогда
			Запись.НомерОпороса = Запись.НомерОпороса + 1;
		КонецЕсли;
		Если ТЧОбщая[КоличествоСтрок - 1].ДатаОпороса <> '00010101000000' И ТЧОбщая[КоличествоСтрок - 1].ДатаОтъема
			= '00010101000000' И (ТЧОбщая[КоличествоСтрок - 1].Живорожденные > 0 Или ТЧОбщая[КоличествоСтрок
			- 1].Мертвые > 0) Тогда
			Отказ = Истина;
...
Процедура ТЧОбщаяНомерЖивотногоХрякПриИзменении(Элемент)
	...
	ТекДанные.НазваниеПородыХряк = _НастройкиКонфигурацииНаСервере.ПолучитьПородуХрякаНаСервере(
		ТекДанные.НомерЖивотногоХряк);
	ТекДанные.НазваниеПородыХряк1 = _НастройкиКонфигурацииНаСервере.ПолучитьПородуХрякаНаСервере(
		ТекДанные.НомерЖивотногоХряк1);
	ТекДанные.НазваниеПородыХряк2 = _НастройкиКонфигурацииНаСервере.ПолучитьПородуХрякаНаСервере(
		ТекДанные.НомерЖивотногоХряк2);
...
&НаСервереБезКонтекста
Процедура ЗаписатьИсториюНомерЖивотногоРегистрНаСервере(ДатаВвода, Знач НаборЗаписей, НомерЖивотного,
	Знач СпрНомерЖивотного)
...
Процедура ДополнительнаяРегистрация(Команда)
	Попытка
		...
		ОткрытьФорму("РегистрСведений.ДополнителнаяРегистрация.Форма.ФормаЗаписи",
			Новый Структура("НомерЖивотного, ДатаВвода", НомерЖивотного, ДатаВвода), , , , , , );
	Исключение 
...
Процедура Медицина(Команда)
	Попытка
		НомерЖивотного 	= Запись.НомерЖивотного;
		ДатаВвода		= Запись.Период;
		ОткрытьФорму("РегистрСведений.Медицина.Форма.ФормаЗаписи", Новый Структура("НомерЖивотного, ДатаВвода",
			НомерЖивотного, ДатаВвода), , , , , , );
	Исключение 
		//Сообщить(Строка(Строка.НомерЖивотного)+"_"+ Строка.Период);
	КонецПопытки;
	ОбновитьИнтерфейс();
...
Процедура ТЧОбщаяВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле.Имя = "ТЧОбщаяКоличествоОтъемышей" Тогда
		ПараметрыФормы = Новый Структура("НомерЖивотного, ДатаВвода, Порода, КоличествоПоросят, Отец, Мать, ПородаОтец, ПородаМать, ДатаРождения",
			Запись.НомерЖивотного, Запись.Период, Запись.КодПороды, ТекущиеДанные.КоличествоОтъемышей,
			ТекущиеДанные.НомерЖивотногоХряк, Запись.НомерЖивотного, ТекущиеДанные.НазваниеПородыХряк, Запись.КодПороды,
			ТекущиеДанные.ДатаОпороса);
