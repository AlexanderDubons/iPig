...
Процедура СформироватьНаСервере(ТабДок)
	...
	Пока КонНедели <= КонецДня(Отчет.ПоДату) Цикл
		...
		НеделяГодаОсеменения = _НастройкиКонфигурацииНаСервере.РасчетНомераНедели(СДаты - 7 * 24 * 60 *30,Константы.НачалоОтсчетаНомераНедели.Получить());
			...
			Если ЗначениеЗаполнено(Выборка.ДатаОсеменения) Тогда
				...
				Если Выборка.ДатаОсеменения >= НачНедели И Выборка.ДатаОсеменения <= КонНедели Тогда 
					Если Выборка.НомерЖивотного <> НомерЖивотного Тогда
						...
						КонецЕсли;
						//Конец				
						Опорос = Выборка.ДатаОсеменения + Константы.ПериодСупоросностиСвиноматки.Получить()*24*60*60;
						...
						Если НЕ ЗначениеЗаполнено(Выборка.ДатаОпороса) Тогда 
							...
							Если Начало <= Выборка.ДатаОсеменения Тогда
								КонецОжОпороса = Выборка.ДатаОсеменения + Константы.ПериодСупоросностиСвиноматки.Получить()*24*60*60;
								Начало = Выборка.ДатаОсеменения;
							Иначе
								КонецОжОпороса = Начало + Константы.ПериодСупоросностиСвиноматки.Получить()*24*60*60;
							КонецЕсли;
							Если Конец >= Выборка.ДатаОсеменения Тогда
								НачалоОжОпороса = Выборка.ДатаОсеменения + Константы.ПериодСупоросностиСвиноматки.Получить()*24*60*60;
								Конец = Выборка.ДатаОсеменения;
							Иначе
								НачалоОжОпороса = Конец + Константы.ПериодСупоросностиСвиноматки.Получить()*24*60*60;
							КонецЕсли;
						КонецЕсли;   
						...
						Если ЗначениеЗаполнено(Выборка.ДатаОпороса) Тогда
							Если Выборка.ДатаОсеменения < Выборка.ДатаОпороса И Выборка.ДатаОпороса < Выборка.ДатаОсеменения + 18*7*24*60*60 Тогда
				...
				Если ЗначениеЗаполнено(Строка.ДатаОсеменения) Тогда 
					...
					Если Строка.ДатаОсеменения >= НачНеделиОсем И Строка.ДатаОсеменения <= КонНеделиОсем Тогда                   
						...
						Если Строка.НомерЖивотного <> НомерЖивотного Тогда
							...
							ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();	
							Пока ВыборкаДетальныеЗаписи.Следующий() Цикл 
								Если (ДатаОс >= НачНеделиОсем И ДатаОС <= КонНеделиОсем) И ДатаОс < ВыборкаДетальныеЗаписи.ДатаОсеменения Тогда
									НоваяСтрока = ПовторкиСписок.Добавить();
									НоваяСтрока.Неделя = ЦЕЛ((ВыборкаДетальныеЗаписи.ДатаОсеменения - ДатаОс)/(24*60*60*7));
									ПоискВыбывших = МассивВыбывших.Найти(Строка.НомерЖивотного);
									Если  ПоискВыбывших = Неопределено Тогда
										НоваяСтрока.Повторка = 1;   
									КонецЕсли;									
									ДобавитьСтрокуТЗВыпавшиеНаСервере(ВыборкаДетальныеЗаписи.ДатаОсеменения, ДатаОс, НеделяГодаОсеменения, НоваяСтрока.Неделя, Строка.НомерЖивотного, Строка.НомерОпороса,Перечисления.ТипВыпадения.Повторы);
						...
						Если ЗначениеЗаполнено(Строка.ДатаАборта) Тогда
							Если Строка.ДатаОсеменения >= НачНеделиОсем И Строка.ДатаОсеменения<= КонНеделиОсем Тогда 
								...
								НоваяСтрока.Неделя 	= ЦЕЛ((Строка.ДатаАборта - Строка.ДатаОсеменения)/(24*60*60*7));  
								ПоискВыбывших = МассивВыбывших.Найти(Строка.НомерЖивотного);
								Если  ПоискВыбывших = Неопределено Тогда
									НоваяСтрока.Аборт 	= 1;            
								КонецЕсли;								
								ДобавитьСтрокуТЗВыпавшиеНаСервере(Строка.ДатаАборта,Строка.ДатаОсеменения, НеделяГодаОсеменения, НоваяСтрока.Неделя, Строка.НомерЖивотного, Строка.НомерОпороса, Перечисления.ТипВыпадения.Аборт);
							КонецЕсли;
						КонецЕсли;  
						Если ЗначениеЗаполнено(Строка.ДатаВыбытие) И НЕ ЗначениеЗаполнено(Строка.ДатаОпороса)  И НЕ ЗначениеЗаполнено(Строка.ДатаУЗИ) Тогда 
							...
							Если Строка.ДатаОсеменения >= НачНеделиОсем И Строка.ДатаОсеменения <= КонНеделиОсем И КоличествоОсеменений = 1 Тогда
								НоваяСтрока = ВыбраковкаСписок.Добавить();
								НоваяСтрока.Неделя = ЦЕЛ((Строка.ДатаВыбытие - Строка.ДатаОсеменения)/(24*60*60*7)); 
								ПоискВыбывших = МассивВыбывших.Найти(Строка.НомерЖивотного);
								Если  ПоискВыбывших = Неопределено Тогда
									НоваяСтрока.Выбраковка = 1; 
								КонецЕсли;								
								ДобавитьСтрокуТЗВыпавшиеНаСервере(Строка.ДатаВыбытие,Строка.ДатаОсеменения, НеделяГодаОсеменения, НоваяСтрока.Неделя, Строка.НомерЖивотного, Строка.НомерОпороса, Перечисления.ТипВыпадения.Выбраковка);
							КонецЕсли;
						КонецЕсли;
					КонецЕсли; 
					...
					Если Строка.РезультатУЗИ = Перечисления.РезультатУЗИ.Негативный И ЦЕЛ((Строка.ДатаУЗИ -Строка.ДатаОсеменения)/(24*60*60*7)) = Неделя Тогда //И КоличествоОсеменений = 1 Тогда
						РезультатПоиска = ТЗВыпавшие.НайтиСтроки(Новый Структура("НомерЖивотного, НеделяОсеменения",Строка.НомерЖивотного,НеделяГодаОсеменения));      
						Если РезультатПоиска.Количество() = 0 Тогда  
							ПоискВыбывших = МассивВыбывших.Найти(Строка.НомерЖивотного);
							Если  ПоискВыбывших = Неопределено Тогда
								Пустые = Пустые + 1;    
							КонецЕсли;
							ДобавитьСтрокуТЗВыпавшиеНаСервере(Строка.ДатаУЗИ,Строка.ДатаОсеменения, НеделяГодаОсеменения, Неделя, Строка.НомерЖивотного, Строка.НомерОпороса, Перечисления.ТипВыпадения.ТестСупоросности);
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
			КонецЦикла;  
		...
		ОбластьСтрокаКонец.Параметры.ОжидаемыеОпоросы = ?(ОжидаемыеОпоросы - КоличествоОпоросов  < 0 ,0 ,ОжидаемыеОпоросы - КоличествоОпоросов ) ;
		...
		Если Всего <> 0  И ОжидаемыеОпоросы - КоличествоОпоросов  > 0 Тогда
			ОбластьСтрокаКонец.Параметры.ПроцентСупоросных = ОКР((ОжидаемыеОпоросы - КоличествоОпоросов) * 100/Всего,2);  
...
&НаСервере
Процедура ДобавитьСтрокуТЗВыпавшиеНаСервере(Знач ДатаВыпадения, Знач ДатаОсеменения, Знач НеделяГодаОсеменения, Знач Неделя, Знач НомерЖивотного, Знач НомерОпороса, Знач ТипВыпадения)
