...
Процедура СформироватьОтчетНаСервере()
	...
	Пока РезультатЗапросаПороды.Следующий() Цикл
		ОбластьШапкаГруппаДанные.Параметры.НомерОпороса = РезультатЗапросаПороды.НомерЖивотного;
...
#Область Воспроизводство
	...
	Пока РезультатЗапросаПороды.Следующий() Цикл
		...
		Запрос.Текст = "ВЫБРАТЬ
					   ...
					   |ГДЕ
					   |	ЖурналОсеменения.Период МЕЖДУ &ДатаНач И &ДатаКон
					   |	И ЖурналОсеменения.НомерЖивотного = &НомерЖивотного
					   ...
					   |	ВТСвиноматок.НомерЖивотного КАК НомерЖивотного
					   ...
					   |		ПО (ВТДнейДоОсеменения.НомерЖивотного = ВТРезультатКоличество.НомерЖивотного)
					   |;
					   |
					   |////////////////////////////////////////////////////////////////////////////////
					   |ВЫБРАТЬ
					   ...
					   |	И Аборт.НомерЖивотного = &НомерЖивотного
					   |;
					   |
					   |////////////////////////////////////////////////////////////////////////////////
					   |ВЫБРАТЬ
					   ...
					   |////////////////////////////////////////////////////////////////////////////////
					   |ВЫБРАТЬ
					   |	ВТВозрастРем.ВозрастРем КАК ВозрастРем
					   |ИЗ
					   |	ВТВозрастРем КАК ВТВозрастРем
					   |ГДЕ
					   |	ВТВозрастРем.НомерЖивотного = &НомерЖивотного";
		Запрос.УстановитьПараметр("ДатаНач", Период.ДатаНачала);
		Запрос.УстановитьПараметр("ДатаКон", Период.ДатаОкончания);
		Запрос.УстановитьПараметр("НомерЖивотного", РезультатЗапросаПороды.НомерЖивотного);
		...
		Пока РезультатЗапроса.Следующий() Цикл
			...
			ЗаполнитьЗначенияСвойств(НоваяСтрока, РезультатЗапроса);
		КонецЦикла;
		РезультатЗапроса = РезультатЗапросаПакет[11].Выбрать();
		Пока РезультатЗапроса.Следующий() Цикл
			ОбластьШапкаВоспроизводствоДанные.Параметры.Заполнить(РезультатЗапроса);
			ЗаполнитьЗначенияСвойств(НоваяСтрока, РезультатЗапроса);
		КонецЦикла;
		РезультатЗапроса = РезультатЗапросаПакет[13].Выбрать();
		Пока РезультатЗапроса.Следующий() Цикл
			ОбластьШапкаВоспроизводствоДанные.Параметры.Заполнить(РезультатЗапроса);
			ЗаполнитьЗначенияСвойств(НоваяСтрока, РезультатЗапроса);
		КонецЦикла;
...
#Область Опорос
	...
	Пока РезультатЗапросаПороды.Следующий() Цикл
		...
		Запрос.Текст = "ВЫБРАТЬ
					   ...
					   |ГДЕ
					   |	ЖурналОпороса.Период МЕЖДУ &ДатаНач И &ДатаКон
					   |	И ЖурналОпороса.НомерЖивотного = &КодПороды
					   |;
					   |
					   |////////////////////////////////////////////////////////////////////////////////
					   |ВЫБРАТЬ
		...
		Запрос.УстановитьПараметр("ДатаНач", Период.ДатаНачала);
		Запрос.УстановитьПараметр("ДатаКон", Период.ДатаОкончания);
		Запрос.УстановитьПараметр("КодПороды", РезультатЗапросаПороды.НомерЖивотного);
...
#Область Производство
	...
	Пока РезультатЗапросаПороды.Следующий() Цикл
		...
		Запрос.Текст = "ВЫБРАТЬ
					   ...
					   |	И ЖурналОсеменения.НомерЖивотного = &КодПороды
					   |;
					   |
					   |////////////////////////////////////////////////////////////////////////////////
					   |ВЫБРАТЬ
...
#КонецОбласти
	...
	ТабЗнач = ПреобразоватьТабличныйДокументВТаблицуЗначений(ТабДок);
КонецПроцедуры
&НаСервере
