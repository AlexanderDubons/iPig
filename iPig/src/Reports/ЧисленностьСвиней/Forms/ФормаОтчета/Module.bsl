		...
		//Хряки
			...
			Иначе
				...
				Пока Выборка.Следующий() Цикл
					Если Выборка.ПоследнееСобытие = Перечисления.ПоследнееСобытие.Случка Или Выборка.ПоследнееСобытие
						= Перечисления.ПоследнееСобытие.УЗИ Тогда
					//Свинки 
						Если ВыборкаДетальныеЗаписи.НомерОпороса = 1 И Не ЗначениеЗаполнено(
							ВыборкаДетальныеЗаписи.ДатаОпороса) Тогда
				...
				//Пустые
					ИначеЕсли Выборка.ПоследнееСобытие = Перечисления.ПоследнееСобытие.Отъем
						Или Выборка.ПоследнееСобытие = Перечисления.ПоследнееСобытие.Аборт Тогда
