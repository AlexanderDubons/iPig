...
#Область Воспроизводство
	...
	Пока НомерОпороса <= МаксНомерОпороса Цикл
		...
		Пока РезультатЗапроса.Следующий() Цикл
			...
			ЗаполнитьЗначенияСвойств(НоваяСтрока, РезультатЗапроса);
		КонецЦикла;
		МассивСлучек.Добавить(?(ЗначениеЗаполнено(ОбластьШапкаВоспроизводствоДанные.Параметры.КоличествоОсеменений),
			ОбластьШапкаВоспроизводствоДанные.Параметры.КоличествоОсеменений, 0));
	...
	Если ТЗДанные.Итог("СвиноматкиОтОтъема") > 0 Тогда
		ОбластьВоспроизводствоИтого.Параметры.ДнейДо1Осеменения = Окр(ТЗДанные.Итог("ДнейОтОтъема") / ТЗДанные.Итог(
			"СвиноматкиОтОтъема"), 2);
	КонецЕсли;
...
#Область Опорос
	...
	Пока НомерОпороса <= МаксНомерОпороса Цикл
		...
		КонецЦикла;
		МассивОпоросов.Добавить(?(ЗначениеЗаполнено(ОбластьШапкаОпоросДанные.Параметры.КоличествоОпоросов),
			ОбластьШапкаОпоросДанные.Параметры.КоличествоОпоросов, 0));
		МассивЖиворожденных.Добавить(?(ЗначениеЗаполнено(ОбластьШапкаОпоросДанные.Параметры.ЖиворожденныеВсего),
			ОбластьШапкаОпоросДанные.Параметры.ЖиворожденныеВсего, 0));
		МассивСлаборожденных.Добавить(?(ЗначениеЗаполнено(ОбластьШапкаОпоросДанные.Параметры.СлаборожденныеВсего),
			ОбластьШапкаОпоросДанные.Параметры.СлаборожденныеВсего, 0));
...
#Область Производство
	...
	Пока НомерОпороса <= МаксНомерОпороса Цикл
		...
		Если (ПродуктивныеДни + НепродуктивныеДни) > 0 Тогда
			...
			ОбластьШапкаПроизводствоДанные.Параметры.ЖиворожденныхНаСвиноматкуВГод = ОКР(ЖиворожденныеНаОпорос
				* ОпоросовНаСвиноматкуВГод, 2);
			ОбластьШапкаПроизводствоДанные.Параметры.ОтнятоПоросятНаСвиноматкуВГод = ОКР(ОтнятыхНаОпорос
				* ОпоросовНаСвиноматкуВГод, 2);
		КонецЕсли;
		Если МассивСлучек[НомерОпороса - 1] > 0 Тогда
			...
			ОбластьШапкаПроизводствоДанные.Параметры.ПроцентОпороса = ОКР(МассивОпоросов[НомерОпороса - 1] * 100
				/ МассивСлучек[НомерОпороса - 1], 2);
		КонецЕсли;
	...
	Если (ПродуктивныеДни + НепродуктивныеДни) > 0 Тогда
		ОпоросовНаСвиноматкуВГод = ОКР(ПроизведеноОпоросов / ((ПродуктивныеДни + НепродуктивныеДни) / 365), 2);
		ОбластьПроизводствоИтого.Параметры.ЖиворожденныхНаСвиноматкуВГод = ОКР(ЖиворожденныеНаОпорос
			* ОпоросовНаСвиноматкуВГод, 2);
		ОбластьПроизводствоИтого.Параметры.ОтнятоПоросятНаСвиноматкуВГод = ОКР(ОтнятыхНаОпорос
			* ОпоросовНаСвиноматкуВГод, 2);
	КонецЕсли;
...
#Область Отъем
	...
	Пока НомерОпороса <= МаксНомерОпороса Цикл
		...
		Если МассивЖиворожденных[НомерОпороса - 1] > 0 Тогда
			ОтнятыхПоросят = ?(ЗначениеЗаполнено(ОбластьШапкаОтъемДанные.Параметры.ОтнятыхПоросят),
				ОбластьШапкаОтъемДанные.Параметры.ОтнятыхПоросят, 0);
			ОбластьШапкаОтъемДанные.Параметры.ПадежДоОтъема = 100 - ОКР(ОтнятыхПоросят
				/ (МассивЖиворожденных[НомерОпороса - 1]) * 100, 2);
		КонецЕсли;
	...
	ОтнятыхПоросят = ?(ЗначениеЗаполнено(ОбластьОтъемИтого.Параметры.ОтнятыхПоросят),
		ОбластьОтъемИтого.Параметры.ОтнятыхПоросят, 0);
...
#Область Результат
	...
	Пока НомерОпороса <= МаксНомерОпороса Цикл
		...
		Иначе
			Запрос.Текст = Запрос.Текст + "	ВЫБОР
										  ...
										  |	КОНЕЦ КАК ПотеряноКормодней,";
		КонецЕсли;
		Запрос.Текст = Запрос.Текст + "	КлючевыеЦифрыОпороса.ПотеряноКормодней КАК ПотеряноКормоднейДляРасчета
									  |ПОМЕСТИТЬ ВТПотеряноДней
									  |ИЗ
									  |	РегистрСведений.КлючевыеЦифрыОпороса КАК КлючевыеЦифрыОпороса
		...
		Пока РезультатЗапроса.Следующий() Цикл
			ПродуктивныеДни = РезультатЗапроса.СупоросныхДней;
			Если РезультатЗапроса.Опоросов > 0 Тогда
				ОбластьШапкаРезультатДанные.Параметры.СупоросныхДнейНаОпорос = Окр(РезультатЗапроса.СупоросныхДней
					/ РезультатЗапроса.Опоросов, 0);
			Иначе
		...
		Пока РезультатЗапроса.Следующий() Цикл
			ПродуктивныеДни = ПродуктивныеДни + РезультатЗапроса.ПодсосныеДни;
			Если РезультатЗапроса.Опоросов > 0 Тогда
				ОбластьШапкаРезультатДанные.Параметры.ПодсосныхДнейНаОпорос = Окр(РезультатЗапроса.ПодсосныеДни
					/ РезультатЗапроса.Опоросов, 0);
				ОбластьШапкаРезультатДанные.Параметры.ВозрастПриОтъемеНаОпорос = Окр(РезультатЗапроса.ВозрастОтъема
					/ РезультатЗапроса.Опоросов, 0);
			...
			КонецЕсли;
		КонецЦикла;
		Если ОбластьШапкаРезультатДанные.Параметры.СупоросныхДнейНаОпорос > 0
			И ОбластьШапкаРезультатДанные.Параметры.ПодсосныхДнейНаОпорос > 0 Тогда
			ПроизведеноОпоросов = ПродуктивныеДни / (ОбластьШапкаРезультатДанные.Параметры.СупоросныхДнейНаОпорос
				+ ОбластьШапкаРезультатДанные.Параметры.ПодсосныхДнейНаОпорос);
		КонецЕсли;
		РезультатЗапроса = РезультатЗапросаПакет[5].Выбрать();
		Пока РезультатЗапроса.Следующий() Цикл
			НепродуктивныеДни = РезультатЗапроса.ПотеряноКормоднейДляРасчета;
			Если ПроизведеноОпоросов > 0 Тогда
				ОбластьШапкаРезультатДанные.Параметры.ПустыеДниНаОпорос = Окр(РезультатЗапроса.ПотеряноКормодней
					/ ПроизведеноОпоросов, 2);
			КонецЕсли;
		КонецЦикла;
		Если (ПродуктивныеДни + НепродуктивныеДни) > 0 Тогда
			ОбластьШапкаРезультатДанные.Параметры.ОпоросовНаСвиноматкуВГод = ОКР(ПроизведеноОпоросов
				/ ((ПродуктивныеДни + НепродуктивныеДни) / 365), 2);
		КонецЕсли;
	...
	Пока РезультатЗапроса.Следующий() Цикл
		ПродуктивныеДни = РезультатЗапроса.СупоросныхДней;
		Если РезультатЗапроса.Опоросов > 0 Тогда
			ОбластьРезультатИтого.Параметры.СупоросныхДнейНаОпорос = Окр(РезультатЗапроса.СупоросныхДней
				/ РезультатЗапроса.Опоросов, 0);
		Иначе
	...
	Пока РезультатЗапроса.Следующий() Цикл
		ПродуктивныеДни = ПродуктивныеДни + РезультатЗапроса.ПодсосныеДни;
		Если РезультатЗапроса.Опоросов > 0 Тогда
			ОбластьРезультатИтого.Параметры.ПодсосныхДнейНаОпорос = Окр(РезультатЗапроса.ПодсосныеДни
				/ РезультатЗапроса.Опоросов, 0);
			ОбластьРезультатИтого.Параметры.ВозрастПриОтъемеНаОпорос = Окр(РезультатЗапроса.ВозрастОтъема
				/ РезультатЗапроса.Опоросов, 0);
		Иначе
		...
		КонецЕсли;
	КонецЦикла;
	Если ОбластьРезультатИтого.Параметры.СупоросныхДнейНаОпорос > 0
		И ОбластьРезультатИтого.Параметры.ПодсосныхДнейНаОпорос > 0 Тогда
		ПроизведеноОпоросов = ПродуктивныеДни / (ОбластьРезультатИтого.Параметры.СупоросныхДнейНаОпорос
			+ ОбластьРезультатИтого.Параметры.ПодсосныхДнейНаОпорос);
	КонецЕсли;
	РезультатЗапроса = РезультатЗапросаПакет[5].Выбрать();
	Пока РезультатЗапроса.Следующий() Цикл
		НепродуктивныеДни = РезультатЗапроса.ПотеряноКормоднейДляРасчета;
		Если ПроизведеноОпоросов > 0 Тогда
			ОбластьРезультатИтого.Параметры.ПустыеДниНаОпорос = Окр(РезультатЗапроса.ПотеряноКормодней
				/ ПроизведеноОпоросов, 2);
		КонецЕсли;
	КонецЦикла;
	Если (ПродуктивныеДни + НепродуктивныеДни) > 0 Тогда
		ОбластьРезультатИтого.Параметры.ОпоросовНаСвиноматкуВГод = ОКР(ПроизведеноОпоросов / ((ПродуктивныеДни
			+ НепродуктивныеДни) / 365), 2);
	КонецЕсли;
