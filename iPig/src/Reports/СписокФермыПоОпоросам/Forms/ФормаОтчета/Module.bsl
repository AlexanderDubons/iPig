...
Процедура ПоОпоросамНаСервере()
	...
	МакетКомпоновки                         =КомпоновщикМакета.Выполнить(Отчеты.ОтчетОДвижениюСырья.ПолучитьМакет(
		"ОсновнаяСхемаКомпоновкиДанных"), Отчет.КомпоновщикНастроек.ПолучитьНастройки(), ДанныеРасшифровкиВ);
...
Процедура СформироватьНаСервере(ТабДок)
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Если ВыборкаДетальныеЗаписи.НомерЖивотного <> НомерЖивотного Тогда
			...
			ИначеЕсли ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.ДатаОпороса) И ВыборкаДетальныеЗаписи.ДатаОпороса
				< Отчет.Дата Тогда
				НомерОпороса = ВыборкаДетальныеЗаписи.НомерОпороса;
